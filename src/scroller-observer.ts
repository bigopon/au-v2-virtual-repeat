...
import { ResizeObserver, ResizeObserverConstructor } from "./resize-observer";
...
export class ScrollerObserverLocator implements IScrollerObsererLocator {
  ...
  /**@internal */
  private readonly cache: WeakMap<HTMLElement, ScrollerObserver> = new WeakMap();
  /**@internal */
...
export class ScrollerObserver implements IScrollerObserver {
  ...
  private readonly subs: Set<IScrollerSubscriber> = new Set();
  private geo: ElementGeometry | null = null;
  /**@internal */
  private sizeObs?: ResizeObserver;
...
class ElementGeometry {
  ...
  public equals(geo: ElementGeometry): boolean {
    if (geo == null) {
      return false;
    }
    return this.t === geo.t
...
const getResizeObserverClass = (p: IPlatform): ResizeObserverConstructor => p.window.ResizeObserver;
